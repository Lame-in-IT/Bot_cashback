...
async def send_photo_file_id(message: types.Message):
    ...
    if id_photo[0] != "No":
        ...
        await bot.send_message(chat_id=1323522063, text=f"Покупатель https://t.me/{message.from_user.username} с номером телефона {id_photo[1]} отправил скриншот.\nНеобходимо посмотреть его на соответствие.".format(message.from_user))
        await bot.send_message(chat_id=540596285, text=f"Покупатель https://t.me/{message.from_user.username} с номером телефона {id_photo[1]} отправил скриншот.\nНеобходимо посмотреть его на соответствие.".format(message.from_user))
        await bot.send_photo(chat_id=1323522063, photo=id_photo[0])
        await bot.send_photo(chat_id=540596285, photo=id_photo[0])
    elif id_photo[0] == "No":
...
async def send_photo_file(message: types.Message):
    ...
    if id_photo[0] != "No":
        await bot.send_message(message.from_user.id, 'Отлично {0.first_name}!\nВаш скриншот принят в обработку и скоро с вами свяжется менеджер'.format(message.from_user), reply_markup=nav.backerMenu)
        await bot.send_message(chat_id=1323522063, text=f"Покупатель https://t.me/{message.from_user.username} с номером телефона {id_photo[1]} отправил скриншот.\nНеобходимо посмотреть его на соответствие.".format(message.from_user))
        await bot.send_document(chat_id=1323522063, document=id_photo[0])
        await bot.send_message(chat_id=540596285, text=f"Покупатель https://t.me/{message.from_user.username} с номером телефона {id_photo[1]} отправил скриншот.\nНеобходимо посмотреть его на соответствие.".format(message.from_user))
        await bot.send_document(chat_id=540596285, document=id_photo[0])
    elif id_photo[0] == "No":
        await bot.send_message(message.from_user.id, 'Простите {0.first_name}, но вы еще не оставили ваш номер телефона.\nПожалуйста нажмите на кнопку "Поделиться номером телефона" и после это отправьте нам свой скриншот. Только после этого вы получите свою вознаграждение.'.format(message.from_user), reply_markup=nav.backerMenu1)
...
async def bot_message(message: types.Message):
    ...
    if read_appeal_text == "3. У меня возник вопрос или проблема":
        ...
        await bot.send_message(chat_id=1323522063, text=f"Пользователь с именем https://t.me/{message.from_user.username} отправил сообщение.\n\n{message.text}".format(message.from_user))
        await bot.send_message(chat_id=540596285, text=f"Пользователь с именем https://t.me/{message.from_user.username} отправил сообщение.\n\n{message.text}".format(message.from_user))
